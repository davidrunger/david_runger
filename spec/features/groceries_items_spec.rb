    ...
    context 'when the user has a spouse' do
      ...
      it 'allows managing store notes and items and checking in a shopping trip', :versioning do
        ...
        within('.modal-container') do
          expect(page).to have_modal_heading("Notes for #{store.name}")
          expect(page).to have_text(store.notes)
          click_on('Edit')
          expect(page).to have_modal_heading("Edit notes for #{store.name}")
      ...
      context "when viewing the spouse's store" do
        ...
        it 'searches and adds items' do
          ...
          within('.modal-container') do
            expect(page).to have_modal_heading("Notes for #{spouse_store.name}")
