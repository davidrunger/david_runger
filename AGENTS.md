...
- When running a feature spec many times without changing frontend assets between runs, prefer compiling the assets once and running against the compiled output. The upfront build cost is generally worthwhile because repeated feature-spec runs are faster without the live Vite server. Rebuild before any subsequent run after changing frontend assets.
