...
- Before running feature specs that exercise changed frontend assets, compile the relevant assets once with `bin/run-test-steps CompileUserJavaScript RunFileSizeChecks` immediately before the specs. Add `CompileAdminJavaScript` when admin assets are relevant. The `RunFileSizeChecks` task completes quickly and can reveal whether the branch requires updating an allowed asset-size range before CI. These tasks centralize the production build configuration and keep this workflow aligned with future compilation changes.
- Do not use the live Vite server for feature specs. Compiled assets avoid dev-server startup, dependency optimization, port ownership, and reload traffic interfering with browser specs, and the Rolldown build is fast enough that compilation is the more reliable default.
- Rebuild after every frontend change before the next feature-spec run; otherwise specs can silently exercise stale compiled assets. When frontend assets do not change between repeated runs, reuse the compiled output. Keep Spring enabled so repeated specs do not pay Rails startup costs.
- Run targeted specs with `bin/rspec path/to/spec.rb` while developing.
- Do not get sidetracked by a local failure in a pre-existing spec outside the active work when the failure is not plausibly related to the current changes and has no known CI occurrence. Rerun it once; if it passes, continue, and if it fails again, report the unrelated local validation failure and continue without investigating or changing it. Repetition only in local runs does not expand the task scope. Investigate flakiness only when it affects a spec under active development, is plausibly related to the current changes, is known from CI, materially impedes development or prevents meaningful validation, or the user asks.
...
- Do not run `bin/ci/run-tests` locally by default. It is primarily a CI command, and CI may catch issues through checks that are intentionally not run during local development.
- When checking asset sizes locally, build and check them with `bin/run-test-steps CompileUserJavaScript CompileAdminJavaScript RunFileSizeChecks`. Do not reuse assets from `RAILS_ENV=test bin/vite build` for the size check: that command uses Vite's test mode and can bypass production-only configuration such as aliases, producing false size violations. Those test-mode assets remain appropriate for feature specs.
...
- Feature specs are relatively expensive to start and exercise a browser. When their behaviors form a coherent user workflow, prefer covering multiple related aspects in one example to avoid redundant page visits. Keep the workflow readable and do not combine unrelated behavior solely to reduce run time.
- In feature specs, assert focus with Capybara's waiting matchers, such as `expect(element).to match_css(':focus')` and `expect(element).not_to match_css(':focus')`, rather than sampling `document.activeElement` via `page.evaluate_script`. These matchers allow browser and frontend focus transitions to complete.
